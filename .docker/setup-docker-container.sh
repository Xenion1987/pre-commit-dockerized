...
function get_user_home() {
  local user_name
  user_name="${1:?}"
  awk -F':' '{print $6}' < <(getent passwd "${user_name}")
}
function get_user_shell() {
  local user_name
  user_name="${1:?}"
  awk -F':' '{print $7}' < <(getent passwd "${user_name}")
}
##############################################
# Install functions
##############################################
function install_ansible_all() {
  install_base_all
...
function install_base_all() {
  install_os_requirements
  install_pip
  install_pip_requirements
  install_pre_commit
}
function install_terraform_all() {
  install_base_all
  install_terraform "${1}"
  install_tf_docs "${2}"
  install_tf_lint "${3}"
...
function install_ansible() {
  local lib_name="ansible"
  local version="${1:-latest}"

  if [[ ${version} != "latest" ]]; then
    lib_name="ansible==${version}"
  fi
...
function install_ansible_lint() {
  local lib_name="ansible-lint"
  local version="${1:-latest}"

  if [[ ${version} != "latest" ]]; then
    lib_name="ansible-lint==${version}"
  fi
...
function setup_direnv() {
  ...
  user_shell="$(get_user_shell "$(whoami)")"
  echo "eval \"\$(direnv hook ${user_shell##*/})\"" >>"${user_home}/.${user_shell##*/}rc"
}
function setup_omb() {
  local theme
  local user_home user_name
  user_name="${1:?}"
  user_home="$(get_user_home "${user_name}")"
  theme="${2:-powerline-multiline}"
...
function help_install() {
 ...
 SUB-COMMANDS:
  ansible-all [VERSION]     - Install ansible
  ansible [VERSION]         - Install ansible
  ansible-lint              - Install ansible-lint
  base-all                  - Install os-requirements, pip, pip-requirements, pre-commit
  help                      - Show [COMMAND] help
  pip                       - Install pip
  ...
  terraform-all
    ...
    [TFLINT_VERSION] \\
    [TFTRIVY_VERSION] \\
    [TERRAGRUNT_VERSION]    - Install all terraform related applications
  terraform-only [VERSION]  - Install terraform VERSION (default: latest)
...
function help_setup() {
 ...
 SUB-COMMANDS:
  direnv USERNAME             - Setup direnv in USERNAME's '.\${SHELL}rc'
  help                        - Show [COMMAND] help
  oh-my-bash USERNAME [THEME] - Setup Oh-My-Bash command line for user USERNAME with theme THEME (default: 'powerline-multiline')
...
function main() {
  ...
  install)
    ...
    terraform-all)
      shift
      install_terraform_all "${1:-latest}" "${2:-latest}" "${3:-latest}" "${4:-latest}" "${5:-latest}"
      ;;
    terraform-docs)
      shift
      install_tf_docs "${1}"
